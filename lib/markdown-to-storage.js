...
export function convertMarkdownToStorage(markdown) {
  const html = markdownIt.render(expandCflmdComments(markdown)).trim();
  ...
  convertInlineMarkdownInTableCells($);
  convertChecklistTextInTableCells($, taskListState);
...
function convertChecklistTextInTableCells($, taskListState) {
  $('td, th').each((_, node) => {
    const cell = $(node);

    if (cell.children('ac\\:task-list').length > 0) {
      return;
    }

    const parsedItems = parseChecklistTextNodes($, cell.contents().toArray());

    if (!parsedItems) {
      return;
    }

    cell.empty().append(renderTaskListMarkup(parsedItems, taskListState));
  });
}
...
function convertInlineMarkdownInTableCells($) {
  $('td, th').each((_, node) => {
    const cell = $(node);
    ...
    if (parseChecklistTextNodes($, cell.contents().toArray())) {
      return;
    }

    for (const child of cell.contents().toArray()) {
      convertInlineMarkdownTextNode($, child);
    }
  });
}

function convertInlineMarkdownTextNode($, node) {
  if (node?.type !== 'text') {
    return;
  }

  if (!node.data.trim() || !containsInlineMarkdownSyntax(node.data)) {
    return;
  }

  const rendered = markdownIt.renderInline(node.data).trim();

  if (!rendered || rendered === escapeHtml(node.data)) {
    return;
  }
...
function containsInlineMarkdownSyntax(text) {
  return (
    /(^|[^*])\*\*[^*]+?\*\*(?!\*)/.test(text) ||
    /(^|[^*])\*[^*\n]+?\*(?!\*)/.test(text) ||
    /`[^`\n]+`/.test(text) ||
...
function parseChecklistTextNodes($, nodes) {
  if (!nodes.some(isMeaningfulNode)) {
    return null;
  }
  ...
  for (const node of nodes) {
    if (node.type === 'text') {
      source += node.data.replace(/\r\n?/g, '\n');
      continue;
    }

    const token = `__CFLMD_TASK_NODE_${tokenIndex}__`;
    tokens.set(token, $.html($(node).clone()));
    source += token;
    tokenIndex += 1;
  }

  return parseChecklistSource(source, tokens);
}

function parseChecklistSource(source, tokens) {
  ...
  for (const line of lines) {
    ...
    if (taskMatch) {
      if (current) {
        items.push(finalizeChecklistItem(current, tokens));
      }

      current = {
        bodyLines: [taskMatch[2]],
        status: taskMatch[1].toLowerCase() === 'x' ? 'complete' : 'incomplete'
      };
      continue;
    }
    ...
    if (/^\s{2,}/.test(line)) {
      current.bodyLines.push(line.replace(/^\s{2}/, ''));
      continue;
    }
...
function finalizeChecklistItem(item, tokens) {
  const bodySource = item.bodyLines.join('\n').trimEnd();
  let bodyHtml = bodySource;

  for (const [token, html] of tokens.entries()) {
    bodyHtml = bodyHtml.replaceAll(token, html);
  }
